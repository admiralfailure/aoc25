type TopDownMap(input: char array array) =
    ...
    member this.Width = Array2D.length2 internalArray

    member this.Height = Array2D.length1 internalArray
...
let getCellsToUpdate (map: TopDownMap) = 
    ...
    let splitters = 
        ...
        |> Array.map(fun (coords, value) ->
            map.GetSides coords
            |> Array.map (fun (sideCoords, value) -> (coords, sideCoords, value))
        )
        |> Array.concat
        |> Array.filter(fun (origCoords, coords, value) -> 
            value = '.'
        )
        |> Array.map (fun (origCoords, coords, value) ->
            origCoords, coords
        )
    ...
    let beams = 
        map.GetAllOfChar '|'
        |> Array.map(fun (coords, value) -> 
            (coords, map.GetBeneath coords)
        )
        |> Array.filter(fun (origCoords, beneath) -> 
            match beneath with
            | [| (beneath_coords, beneath_value) |] -> beneath_value = '.'
            | _ -> false
        )
        |> Array.map (fun (origCoords, beneath) ->
...
let tick (map: TopDownMap) (routes: int64 array) = 
    match getCellsToUpdate map with
    | [|splitters; beams; spawners|] -> 
        splitters
        |> Array.iter(fun ((origX, origY), (x, y)) -> 
            map[x, y] <- '|'
            routes[y] <- routes[origY] + routes[y]
        )
        ...
        (routes, splitters.Length + beams.Length + spawners.Length)
    | _ -> ([||], 0)

let process (map: TopDownMap) =
    let rec simulate map colRoutes =
        let (routes, total) = tick map colRoutes
    ...
    simulate map (Array.init (map.Width) (fun x -> 0L))
